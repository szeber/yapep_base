...
class ViewDo
{
    ...
    public function checkIsEmpty($key, $checkIsSet = false)
    {
        ...
        return $checkIsSet ? !array_key_exists($keyParts[count($keyParts) - 1],
            $target) : empty($target[$keyParts[count($keyParts) - 1]]);
    }
    ...
    public function checkIsArray($key)
    {
        ...
        return isset($target[$keyParts[count($keyParts) - 1]]) ? is_array($target[$keyParts[count($keyParts) - 1]]) : false;
    }
    ...
    protected function escapeForHtml($value)
    {
        switch (gettype($value)) {
            ...
            case 'object':
                if (($value instanceof \ArrayAccess) && (($value instanceof \Iterator) || ($value instanceof \IteratorAggregate))) {
