...
class LdapConnection
{
    ...
    public function setAttributeValue(LdapDn $dn, $attributeName, $value)
    {
        ...
        $result = @ldap_mod_replace($this->link, (string)$dn, [$attributeName => $value]);
    ...
    public function addAttributeValues(LdapDn $dn, $attributeName, array $valuesToAdd, $uniqueValues = true)
    {
        ...
        $entry = $this->search($dn, 'objectclass=*', [], ['cn', $attributeName]);

        if (empty($entry[0]['cn'])) {
            return false;
        }

        $currentValues = isset($entry[0][$attributeName]) ? $entry[0][$attributeName] : [];
        $newValues     = array_merge($currentValues, $valuesToAdd);

        $newValues = $uniqueValues ? array_unique($newValues) : $newValues;

        $result = @ldap_mod_replace($this->link, (string)$dn, [$attributeName => array_values($newValues)]);
    ...
    public function deleteAttributeValues(LdapDn $dn, $attributeName, array $valuesToDelete)
    {
        ...
        $entry = $this->search($dn, 'objectclass=*', [], [$attributeName, 'cn']);
        ...
        $result = @ldap_mod_replace($this->link, (string)$dn, [$attributeName => $newValues]);
    ...
    public function search(
        ...
        $filterParams = [],
        $attributes = [],
        $deref = self::DEREF_NEVER,
        $scope = self::SCOPE_SUB
    ) {
        if (!$this->link) {
            $this->connect();
        }
