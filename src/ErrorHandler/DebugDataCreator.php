...
class DebugDataCreator implements IErrorHandler
{
    ...
    public function handleException($exception, $errorId)
    {
        ...
        $this->storage->set($errorId, $this->getDebugData(
            $errorId,
            $errorMessage,
            ($this->isTestMode ? [] : $exception->getTrace()),
            ($this->isTestMode ? [] : ['exception' => $exception])
        ));
    }
     ...
     * @return string
     */
    protected function getDebugData($errorId, $errorMessage, array $backTrace = [], array $context = [])
    {
